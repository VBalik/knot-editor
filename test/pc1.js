(async ()=>{
  ...
  const out={};
  const run=(prep,w,r)=>{ prep(); H.setSlider('thick',w); H.setSlider('repCoef',r); const rr=runTo(800); return Object.assign(rr, metrics()); };
  out.presets={}; for(const k of ['trefoil','figure8','cinquefoil','septafoil']) out.presets[k]=run(()=>H.clickPreset(k),5,2);
  out.trefoil_w={}; for(const w of [1,3,5,7,10]) out.trefoil_w['w'+w]=run(()=>H.clickPreset('trefoil'), w, 2);
  out.trefoil_r={}; for(const r of [1,4,7,10]) out.trefoil_r['r'+r]=run(()=>H.clickPreset('trefoil'), 3, r);
  out.same_s={}; for(const [w,r] of [[1,10],[10,1],[5,6]]) out.same_s['w'+w+'r'+r]=run(()=>H.clickPreset('trefoil'), w, r);
  out.max={}; for(const k of ['trefoil','septafoil']) out.max[k]=run(()=>H.clickPreset(k),10,10);
  return out; })()
