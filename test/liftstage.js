(async ()=>{ const H=global.__H; window.__noAutoSave=true; const fs=global.__require('fs'), vm=global.__require('vm');
  ...
  for(const i of IDS){ let seed=(20260903+i*7919)&0x7fffffff; const rnd=()=>{ seed=(seed*1103515245+12345)&0x7fffffff; return seed/0x7fffffff; };
    ...
    window.__liftStages={}; document.getElementById('clear').onclick(); RK.drawPts(H, best.pts, best.start);
    ...
    const d=H.dbg(); const st=window.__liftStages;
    fs.appendFileSync(OUT, JSON.stringify({i, nc, N:d.N, det2d:d.knotDet, fixed:st.fixed, pre:st.pre, post:st.post, smooth:smooth.map(p=>[+p.x.toFixed(3),+p.y.toFixed(3)]), crossings:crossings.map(c=>Object.assign({},c))})+'\n'); }
  return 'ok'; })()
